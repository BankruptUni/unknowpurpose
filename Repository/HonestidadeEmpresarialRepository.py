...
import csv
import traceback
class HonestidadeEmpresarialRepository:
    ...
    def GetAllFineCompany(self) -> str:
        ...
        try:        
            ...
            with open(f"{self._diretorio}/sancoes.csv", "r", encoding = "utf-8-sig") as sancoes:
                ...
                print("\n".join([f"—{empresa} : R$ {valor}"if HonestidadeEmpresarialRepository.GetCurrency(valor) else f"—{empresa} : {valor}"  for empresa, valor in valores]))
        except Exception:
            traceback.print_exc()
        return valores[-1][1]       